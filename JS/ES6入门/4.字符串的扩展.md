...
但是这个方法在现在新版的chrome里面还是不行的

![](https://ws4.sinaimg.cn/large/006tKfTcgy1fg7udaubw7j30q6058mxb.jpg)
...
![](https://ws4.sinaimg.cn/large/006tKfTcgy1fg7w2qsronj30hi06kmx2.jpg)

这三个方法都支持第二个参数,表示开始搜索的位置,但是endsWith不一样,他是针对前n个字符,而其它两个方法针对从第n个位置直到字符串结束
...
console.log('x'.padEnd(4,'ab'));
```
![](https://ws4.sinaimg.cn/large/006tKfTcgy1fg7wm4zdw7j30dc07q747.jpg)
...
![](https://ws1.sinaimg.cn/large/006tKfTcgy1fg7zwtiol8j30qm030dfs.jpg)
...
```
![](https://ws3.sinaimg.cn/large/006tKfTcgy1fg80yjr37qj30v60cwgly.jpg)
...
标签模板其实不是模板,而是函数调用的一种特殊形式,"标签"指的就是函数,紧跟在后面的模板字符串就是它的参数,但是,如果模板字符串有变量,就不是简单的调用了,而是会将模板字符串先处理成多个参数,再调用函数
...
![](https://ws4.sinaimg.cn/large/006tKfTcgy1fg82ym20zjj30ru0a8mxc.jpg)
...
String.raw方法,往往用来充当模板字符的处理函数,返回一个斜杠都被转义,(即斜杠前面再加一个斜杠)的字符串,对应替换变量后的模板字符串
...
```
但是我在chrome里面运行时这样的,并没有加上\

![](https://ws2.sinaimg.cn/large/006tKfTcgy1fg84dsv1azj30jw08cmx4.jpg)
